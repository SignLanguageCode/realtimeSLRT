...
class LSTM_seq(nn.Module):
    ...
    def forward(self, x, label):
        ...
        # h = h+h1+h2
        # c = c+c1+c2
        # h = F.leaky_relu(self.memory_cell3(torch.cat((h[-2, :, :], h[-1, :, :]), dim=1))).unsqueeze(0)
        # c = F.leaky_relu(self.memory_cell3(torch.cat((c[-2, :, :], c[-1, :, :]), dim=1))).unsqueeze(0)
        ...
        # h = (torch.add(h[-2, :, :], h[-1,:, :])/2).unsqueeze(0)
        # c = (torch.add(c[-2, :, :], c[-1,:, :])/2).unsqueeze(0)
        # h = [batch, feature]
...
if __name__ == '__main__':
    ...
    biLSTM = LSTM_seq(max_seq=10, input_size=4096, hidden_size=2048, class_num=10).to(device)
    input1 = torch.rand(size=(3,24, 4096), dtype=torch.float32).to(device)

    o1= biLSTM(input1, label=torch.tensor([[0, 1, 2, 2, 2, 3, 3, 3, 3, 3],
                                           [0, 1, 2, 2, 2, 3, 3, 3, 3, 3],
                                           [0, 1, 2, 2, 2, 3, 3, 3, 3, 3]]).to('cuda'))
    print(o1.shape)